    ...
    public boolean a() {
        ...
        if (list.isEmpty())
            ...
            if (this.d instanceof EntityHuman) {
                ...
                PlayerManager PlayerManager = UhcHost.getInstance().getPlayerManager(eh.getBukkitEntity().getUniqueId());
                return eh.getBukkitEntity().getLocation().distance(this.e.getBukkitEntity().getLocation()) < 8.0D && eh.getBukkitEntity().getGameMode() != GameMode.SPECTATOR && PlayerManager.isAlive();
            }
        }
        return false;
    }
