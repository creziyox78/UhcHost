...
import fr.maygo.uhc.modes.naruto.v2.items.ShurikenJutsuItem;
...
public class Minato extends Role implements NarutoV2Role, KuramaItem.KuramaUser, RoleCommand, RoleListener, BijuUser {
    ...
    public void onKill(OfflinePlayer player, Player killer) {
        ...
        if (PlayerManager.getCamps() != main.getPlayerManager(killer.getUniqueId()).getCamps()) {
    ...
    public void onEntityShootBow(EntityShootBowEvent event) {
        if (event.getEntity() instanceof Player) {
            ...
            if (PlayerManager.getRole() instanceof Minato) {
                ...
                if (PlayerManager.getRoleCooldownShurikenJustu() == 0) {
                    if (bow.hasItemMeta() && bow.getItemMeta().hasDisplayName()) {
                        if (bow.getItemMeta().getDisplayName().equalsIgnoreCase("§cShurikenJutsu")) {
                            ...
                            PlayerManager.setRoleCooldownShurikenJustu(15);
                            PlayerManager.sendTimer(player, PlayerManager.getRoleCooldownShurikenJustu(), bow);
                        }
                    }
                }
            }
