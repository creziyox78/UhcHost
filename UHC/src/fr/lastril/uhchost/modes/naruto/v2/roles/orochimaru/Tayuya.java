...
import fr.lastril.uhchost.modes.naruto.v2.items.FluteDemoniaque;
...
public class Tayuya extends Role implements NarutoV2Role, RoleListener, MarqueMauditeItem.MarqueMauditeUser {
			...
			if (golem.getCustomName().equalsIgnoreCase("§dServant de Tayuya")) {
				if(golem.getTarget() != null) {
					if(golem.getTarget() instanceof Player) {
						PlayerManager joueur = main.getPlayerManager(golem.getTarget().getUniqueId());
						if(joueur.hasRole()) {
			...
			if (endermite.getCustomName().equalsIgnoreCase("§dServant de Tayuya")) {
				if(endermite.getTarget() != null) {
					if(endermite.getTarget() instanceof Player) {
						PlayerManager joueur = main.getPlayerManager(endermite.getTarget().getUniqueId());
						if(joueur.hasRole()) {
							if(joueur.getRole() instanceof Tayuya) {
	...
	public void onUseMarqueMaudite(UhcHost main, Player player, PlayerManager joueur) {
		if(player.getMaxHealth() - 2D <= 0){
			...
			joueur.setRoleCooldownMarqueMaudite(60 * 10);
