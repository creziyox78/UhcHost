...
public abstract class Role implements RoleDescription {
	...
	public void sendDescription(Player player) {
		...
		if (!this.getRoleToKnow().isEmpty()) {
			for (Class<? extends Role> roleToKnow : this.getRoleToKnow()) {
				/*if(!UhcHost.getInstance().getNarutoManager().getJoueursWithRole(roleToKnow).isEmpty()) {
					try {
						player.sendMessage( "§eLe(s) joueur(s) possédant le rôle §6" + roleToKnow.newInstance().getRoleName() + "§e :");
					} catch (InstantiationException | IllegalAccessException e) {
						e.printStackTrace();
					}
					for (PlayerManager joueurHasRole : UhcHost.getInstance().getNarutoManager().getJoueursWithRole(roleToKnow)) {
	...
	public abstract String getSkullValue();
	...
	public abstract ItemsCreator getItem();
